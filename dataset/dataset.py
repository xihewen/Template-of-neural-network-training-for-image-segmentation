...
if __name__ == '__main__':
    ct_path = r'../data_prepare/train/ct'
    seg_path = r'../data_prepare/train/seg'
    dataset = Dataset(ct_path, seg_path)
    train_loader = DataLoader(dataset, 2, True, num_workers=1, pin_memory=True)  # 这样会出错，dataloader中数据大小size要一致！！！
    for index, (ct, seg) in enumerate(train_loader):
        print(type(ct), type(seg))
        print(index, ct.size(), seg.size(),ct)
        print('----------------')
        break